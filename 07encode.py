...
def conver_sensor2dict(set_sensors, data_name, distant_int, opts):
    ...
    with open(os.path.join(opdir, 'sensor2dict.txt'), 'w', encoding="utf-8") as fw:
        json.dump(dict_sensor2serial_number, fw)
...
def encoder_sensors(read_sensor2dict, data_dir, data_name, distant_int, opts, debuge=True):
    ...
    for str_activitydata in list_activitydata:
        list_ids = []
        with open(os.path.join(data_dir, data_name, str_activitydata), 'r', encoding="utf-8") as fr:
            lines = fr.readlines()
            list_id = []
            for i, line in enumerate(lines):
                f_info = line.split()
                if len(f_info) >= 5:
                    ...
                    if sensor[0] == 'M' or sensor[0] == 'D' or sensor[0] == 'I' or sensor[
                        ...
                        if f_info[4] in ['ON', 'OFF', 'ABSENT', 'PRESENT', 'OPEN', 'CLOSE']:
                            ...
                            print(
                                'This data is abnormal status data, which may be marked incorrectly\ N data is: %s' % line)
                    elif sensor[0] == 'A' or sensor[0] == 'P' or sensor[0] == 'T':  # ON
                        list_id.append(read_sensor2dict[f_info[3]])
                    elif sensor[0] == 'E':
                        pass
                    else:
                        print(
                            'dataset：%s\tactivity:%s\trow:%d data:%s\n' % (data_name, str_activitydata, i, line))
                else:
    ...
    with open(os.path.join(opdir, data_name + '-dict_ids.txt'), 'w', encoding="utf-8") as fw:
        json.dump(dict_ids, fw)
...
def decoder_sensors(read_sensor2dict, data_dir, data_name, opts, debuge=True):
    ...
    with open(os.path.join(opdir, data_name + '-dict_sensors.txt'), 'w') as fw:
        json.dump(dict_sensors, fw)
...
if __name__ == '__main__':
    ...
    for i in range(7):
        ...
        for data_name in data_names:
            ...
            encoder_dict_path = os.path.join(opts["datasets"]["base_dir"], 'ende', data_name, str(distant_int),
                                             'sensor2dict.npy')
            ...
            # dict_ids = np.load(ids_dir + '\\' + ids_name, allow_pickle=True, allow_pickle=True).item()
